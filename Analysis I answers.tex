...
\begin{document}
	...
	First, we prove a lemma: the statement that if there exists a function $f$ from $S$ to $\emptyset$, then $S = \emptyset$. We will prove this using proof by contradiction. If $S$ is nonempty, then there exists an element of $S$, which we will call $s$. But then it is impossible to have an element of $\emptyset$, so $f(s)$ cannot exist. Therefore, we have a contradiction, and $S = \emptyset$.
	
	By Lemma 3.4.10, there exists $Y'' = \{Y': Y' \text{ is a subset of } Y\}$. By Axiom 3.7 (the axiom of replacement) and Axiom 3.11 (the power set axiom), there exists a set \[Z = \{{Y^\prime}^X: Y' \in Y''\}.\] We can use it again to get, where $f|_{X'}$ denotes, if $X' \subseteq X$, the function from $X'$ to $Y$ where for any $x \in X'$, $f|_{X'}(x) \coloneqq f(x)$: \[Z' = \{ \, \{f|_{X'}: X' \in X\}: f \in Z\}.\] Using the axiom of union (Axiom 3.12), we can create $Z'' = \bigcup Z'$. It is obvious that $f \in Z''$ implies that $f$ is a partial function from $X' \in X$ to $Y' \in Y$. Finally, we will show that $f$ being a partial function from $X' \in X$ to $Y' \in Y$ implies that $f \in Z''$. We know that $Y' \in Y''$. Therefore, $g \colon X \rightarrow Y'$ defined by $g(x) \coloneqq
	\begin{cases}
		f(x) & \text{if } x \in X' \\
		y & \text{otherwise}
	\end{cases}$
	where $y$ is in $Y'$, assuming that $Y' \ne \emptyset$. (If $Y'$ is empty, then by the lemma, $X$ is also empty, and $f$ and $g$ can be defined to be the same function, the empty function from $\emptyset$ to $Y'$. Therefore, ).
	
	\subsection*{Exercise 3.4.9}
	
	
	
	\subsection*{Exercise 3.4.11}
