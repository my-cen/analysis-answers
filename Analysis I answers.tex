...
\begin{document}
	...
	\noindent (a) First, we prove that $x^n x^m = x^{n + m}$. We induct on $m$. When $m = 0$, $x^m = 1$, and hence, $x^n x^m = x^n$. But $x^{n + m} = x^{n + 0} = x^n$, and therefore, $x^n x^m = x^{n + m}$ is true when $m = 0$. When $x^n x^m = x^{n + m}$,
