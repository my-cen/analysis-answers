...
\begin{document}
	...
	By Lemma 3.4.10, there exists $Y'' = \{Y': Y' \text{ is a subset of } Y\}$. By Axiom 3.7 (the axiom of replacement), there exists a set \[Z = \{{Y^\prime}^X: Y' \in Y''\}.\] We can use it again to get, where $f|_{X'}$ denotes, if $X' \subseteq X$, the function from $X'$ to $Y$ where for any $x \in X'$, $f|_{X'}(x) \coloneqq f(x)$: \[Z' = \]
